    ...
    try {
      switch (name) {
        case 'voicememos_open': {
          ...
          return {
            content: [
              { type: 'text' as const, text: 'Voice Memos app opened' },
            ],
          };
        }
        ...
        case 'voicememos_list': {
          ...
          try {
            ...
            if (files.length === 0) {
              return {
                content: [
                  { type: 'text' as const, text: 'No voice memos found' },
                ],
              };
            }
  ...
  tell process "Voice Memos"
    click button 1 of window 1
          ...
          try {
            runAppleScript(script);
            return {
              content: [{ type: 'text' as const, text: 'Stopped recording' }],
            };
          } catch {
            return {
              content: [
        ...
        case 'voicememos_play': {
          ...
          try {
            ...
            return {
              content: [
                { type: 'text' as const, text: `Playing: ${filename}` },
              ],
            };
          } catch {
            return {
    ...
    describe('voicememos_list', () => {
      ...
      it('should handle folder access error', async () => {
        ...
        expect(textContent?.text).toContain(
          'Could not access Voice Memos folder'
        );
